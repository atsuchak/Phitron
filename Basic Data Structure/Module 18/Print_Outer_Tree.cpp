...
void print_lft(Node* root) {
    if(!root) return;

    if(root->left) print_lft(root->left);
    else if(root->right) print_lft(root->right);
    cout << root->val << " ";
}

void print_rght(Node* root) {
    if(!root) return;

    cout << root->val << " ";
    if(root->right) print_rght(root->right);
    else if(root->left) print_rght(root->left);
}
