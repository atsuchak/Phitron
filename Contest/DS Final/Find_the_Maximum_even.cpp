...
//Problem link: https://www.hackerrank.com/contests/early-problem-solvers-club-selection-contest-batch-08-a-2k26/challenges/maximum-even-value
...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        a[i]%2 == 0? ev.push_back(a[i]) : od.push_back(a[i]);
    }
    ...
    if(ev.size() == 1) mx = max(mx, ev[0]);
    if(ev.size() > 1) mx = max(mx, ev[0]+ev[1]);
    if(od.size() > 1) mx = max(mx, od[0]+od[1]);
