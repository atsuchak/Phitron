...
//Problem link: https://www.hackerrank.com/contests/early-problem-solvers-club-selection-contest-batch-08-a-2k26/challenges/undo-redo-3
...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(st == "TYPE") {
            ...
            if(!store.empty()) {
                redo.push_back(store.back());
                store.pop_back();
            }
        } else if(st == "REDO") {
            if(!redo.empty()) {
                store.push_back(redo.back());
                redo.pop_back();
            }
        }
    }
